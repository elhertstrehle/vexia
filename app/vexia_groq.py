...
# VexIA - Mestre de RPG para Star Wars: Imperial Assault
# Versão: 13 - Jun/2025 - Melhorias no roteador semântico e palavras-chave
...
if pergunta:
    ...
    elif any(p in pergunta_lower for p in ["descansar", "descanso", "ação de descanso", "curar", "recuperar", "tensão"]):
    ...
    if tema_forcado:
        ...
        textos = [f"{b.get('tema', '')} {b.get('descricao', '')} {b.get('texto', '')}" for b in blocos]
        textos_embed = modelo.encode(textos, convert_to_tensor=True)
        pergunta_embed = modelo.encode(pergunta, convert_to_tensor=True)
        sim_scores = util.cos_sim(pergunta_embed, textos_embed)[0]
